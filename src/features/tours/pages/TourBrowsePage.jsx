...
function mapApiTourToCard(apiTour) {
  ...
  return {
    ...
    matchTag: apiTour.destination ? `Explore ${apiTour.destination}` : null,
    tags: apiTour.activities
      ...
      : apiTour.destination
        ? [apiTour.destination]
        : [],
    guide: apiTour.guide?.fullName ?? apiTour.guide?.name ?? "Local guide",
    avatar:
...
export default function TourBrowsePage() {
  ...
  const hasFilters =
    search || destination || groupType || minPrice || maxPrice || sortBy;
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:py-8">
        ...
        {showFilters && (
          <div className="rounded-xl sm:rounded-2xl border border-[#dddced] bg-white p-4 sm:p-5 mb-5 space-y-4">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div>
                <label className="block text-[10px] sm:text-xs font-semibold text-[#5d5b84] mb-1">
                  Search
                </label>
                <div className="relative">
                  <Search
                    size={14}
                    className="absolute left-3 top-1/2 -translate-y-1/2 text-[#aaaacf]"
                  />
                  <input
                    value={search}
              ...
              <div>
                ...
                <input
                  value={destination}
                  onChange={(e) =>
                    updateParams({ destination: e.target.value })
                  }
              ...
              <div>
                ...
                <select
                  value={groupType}
                  onChange={(e) => updateParams({ groupType: e.target.value })}
                  className="w-full rounded-lg border border-[#d5d4ea] bg-white px-3 py-2 text-xs sm:text-sm outline-none focus:ring-2 focus:ring-[#302d96]"
                >
                  {GROUP_TYPES.map((gt) => (
        ...
        {loading ? (
          ...
          <div className="rounded-2xl bg-red-50 px-6 py-10 text-center">
            ...
            <button
              onClick={() => window.location.reload()}
              className="mt-4 rounded-full bg-[#010170] px-5 py-2 text-xs font-semibold text-white"
            >
              Retry
            </button>
          </div>
        ) : tours.length === 0 ? (
          <div className="py-20 text-center text-[#353572] text-sm">
            No tours found.
          </div>
        ) : (
          <ToursSection />
