...
export function OnboardingNavbar() {
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-[rgba(1,1,112,0.25)] shadow-[0_8px_24px_rgba(1,1,112,0.1)]">
      <div className="max-w-[1728px] mx-auto px-10 h-16 flex items-center justify-between">
        {/* Logo */}
        <span
          className="text-2xl font-['EB_Garamond',serif] tracking-widest"
          style={{
        ...
        {/* Role badge */}
        <div className="flex items-center gap-2 bg-[#f4f4f8] border border-[#aaaacf] rounded-full px-5 py-2 text-sm text-[#010170] font-['Inter',sans-serif]">
          <img src={localGuideIcon} alt="" className="w-5 h-5 object-contain" />
          Joining as a tourist
        </div>
      </div>
    </header>
  );
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      {status === "done" ? (
        ...
        >
          <img src={checkIcon} alt="done" className="w-7 h-7 object-contain" />
        </div>
      ) : status === "active" ? (
        <div className="p-2 rounded-full border-[6px] border-[rgba(237,200,76,0.18)]">
          <div className="w-14 h-14 rounded-full flex items-center justify-center border-[5px] border-[#edc84c]">
            <span className="text-lg font-semibold text-[#010138]">{number}</span>
          </div>
        </div>
      ) : (
        <div className="w-14 h-14 rounded-full flex items-center justify-center border-[5px] border-[#cccce2]">
          <span className="text-lg font-semibold text-[#cccce2]">{number}</span>
        </div>
      )}
      <span
    ...
    <footer className="border-t border-[rgba(53,53,114,0.25)] shadow-[0_8px_24px_rgba(53,53,114,0.15)] bg-white">
      <div className="max-w-[1728px] mx-auto px-10 h-16 flex items-center justify-between">
        ...
        {backTo ? (
          <Link
            to={backTo}
            className="flex items-center gap-2 text-base text-[#cccce2] hover:text-[#010170] transition-colors"
          >
            <img src={leftArrowIcon} alt="" className="w-5 h-5 object-contain" />
            <span>Back</span>
          </Link>
        ) : (
          <div />
        ...
        <div className="flex items-center gap-5">
          {(skipLabel && onSkip) && (
            <button
              onClick={onSkip}
              className="text-base font-medium text-[#010170] hover:underline"
            >
              {skipLabel}
            </button>
          )}
